      ...
      if (isClient) {
        if (!clientLogoutRequest) {
          clientLogoutRequest = fetch("/api/auth/logout", {
            method: "POST",
            body: null, // Logout luon luon thanh cong
            headers: {
              ...baseHeaders,
            } as any,
          });
          try {
            ...
            // co the dan den loop vo hang neu khong duoc xu ly dung cach
            // vi neu roi vao truong hop tai trang login, cta co goi cac api can access token
            // ma access token da bi xoa, thi se bi redirect ve trang login va cu the no se bi lap
            location.href = "/login";
          }
        }
      } else {
